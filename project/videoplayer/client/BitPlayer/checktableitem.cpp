    ...
    , videoInfo(videoInfo)
{
    ...
    ui->videoTittleLabel->setText(videoInfo.videoTitle);
    ...
    // 操作按钮2信号槽
    connect(ui->operationBtn2, &QPushButton::clicked, this, &CheckTableItem::onOperator2BtnClicked);
...
void CheckTableItem::onVideoBtnClicked()
{
    ...
    playerPage->setPlayPage("systemPage");

    // 设置用户头像
    if(videoInfo.userAvatarId.isEmpty()){
        // 加载默认的头像
        userAvatar = makeCircleIcon(loadFileToByteArray(":/images/myself/defaultAvatar.png"), 13).pixmap(26, 26);
...
{
    // 操作按钮，操作按钮上的文本可能是：通过(状态按钮：待审核)  上架(状态按钮：已下架)  下架(状态按钮：已审核)
...
void CheckTableItem::onOperator2BtnClicked()
{
    auto dataCenter = model::DataCenter::getInstance();
    ConfirmDialog* confirmDlg = new ConfirmDialog();
    ...
    if(confirmDlg->getConfirm()){
        ...
        // 修改DataCenter中的视频审核信息
        auto statusVideoList = dataCenter->getStatusVideoList();
        statusVideoList->updateVideoCheckInfo(videoInfo.videoId, model::VideoStatus::reject, myselfInfo->nickname,
                                              myselfInfo->userId, myselfInfo->avatarFileId);
...
{
    if(userId != videoInfo.userAvatarId){
        return;
    }
